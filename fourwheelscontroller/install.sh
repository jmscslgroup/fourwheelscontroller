...
echo "=========================="
echo "Installing App fourwheelscontroller"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for fourwheelscontroller..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/fourwheelscontroller/installRosPackages.sh
...
rosrun robot_upstart install fourwheelscontroller/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable fourwheelscontroller
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
